...
5. Run recovery phrase e2e coverage when a test database is available.
6. Validate the Postman collection still parses as JSON.
...
| Recovery phrase e2e   | `npm run test:e2e -- test/auth/recovery-phrase.e2e-spec.ts`                                                                                                             | Verifies user generate/status/recover, generic wrong/reused failures, and admin reset clearing |
...
5. Run the **Recovery Phrase Flow** folder to generate a phrase, capture it into a collection variable at runtime, recover the account, and verify reuse fails.
6. Run the **Role & Error Coverage** folder to verify `SALES`/`MECHANIC` restrictions plus invalid login.
...
- `Recovery Setup Create Sales User` → `Recovery User Login` → `Recovery Generate Phrase` → `Recovery Status` → `Recovery With Captured Phrase` → `Recovery Reuse Fails`
...
The recovery phrase collection variable starts empty and is populated from the generation response during the run. Do not paste real recovery phrases into the committed collection.
...
- [ ] Recovery phrase coverage avoids committed plaintext phrase values and confirms admin reset clearing in automated e2e.
